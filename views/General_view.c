...
// View Model - holds all view state in a thread-safe structure
typedef struct {
    general_view current_view;
    carousel_info info_selector;
...
static void _draw_view_sensorsList(Canvas* canvas, GeneralViewModel* model) {
    //Текущая страница
    uint8_t page = model->sensor_index / 4;
...
static void _draw_carousel_values(Canvas* canvas, GeneralViewModel* model) {
    UnitempStatus sensor_status = unitemp_sensor_getActive(model->sensor_index)->status;
    if(sensor_status == UT_SENSORSTATUS_ERROR || sensor_status == UT_SENSORSTATUS_TIMEOUT) {
        ...
        if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &SINGLE_WIRE) {
            snprintf(
                app->buff,
                BUFF_SIZE,
                "Waiting for module on pin %d",
                ((SingleWireSensor*)unitemp_sensor_getActive(model->sensor_index)->instance)
                    ->gpio->num);
        }
        if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &ONE_WIRE) {
            snprintf(
                app->buff,
                BUFF_SIZE,
                "Waiting for module on pin %d",
                ((OneWireSensor*)unitemp_sensor_getActive(model->sensor_index)->instance)
                    ->bus->gpio->num);
        }
        if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &I2C) {
            snprintf(app->buff, BUFF_SIZE, "Waiting for module on I2C pins");
        }
        if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &SPI) {
    ...
    switch(unitemp_sensor_getActive(model->sensor_index)->type->datatype) {
    case UT_DATA_TYPE_TEMP:
        _draw_temperature(
            canvas,
            unitemp_sensor_getActive(model->sensor_index),
            temp_positions[0][0],
            temp_positions[0][1],
            ColorWhite);
        break;
    case UT_DATA_TYPE_TEMP_HUM:
        _draw_temperature(
            ...
            ColorWhite);
        _draw_humidity(
            canvas, unitemp_sensor_getActive(model->sensor_index), hum_positions[0]);
        break;
    case UT_DATA_TYPE_TEMP_PRESS:
        _draw_temperature(
            ...
            temp_positions[1][1],
            ColorWhite);
        _draw_pressure(canvas, unitemp_sensor_getActive(model->sensor_index));
        break;
    ...
    case UT_DATA_TYPE_TEMP_HUM_PRESS:
        _draw_temperature(
            ...
            ColorWhite);
        _draw_humidity(
            canvas, unitemp_sensor_getActive(model->sensor_index), hum_positions[1]);
        _draw_pressure(canvas, unitemp_sensor_getActive(model->sensor_index));
        break;
    case UT_DATA_TYPE_TEMP_HUM_CO2:
        _draw_temperature(
            ...
            temp_positions[2][1],
            ColorWhite);
        _draw_humidity(canvas, unitemp_sensor_getActive(model->sensor_index), hum_positions[1]);
        _draw_co2(canvas, unitemp_sensor_getActive(model->sensor_index), ColorWhite);
        break;
    case UT_DATA_TYPE_PM:
        _draw_pm_1_0(canvas, unitemp_sensor_getActive(model->sensor_index), ColorWhite, value_positions[3][0], QUARTER);
        _draw_pm_2_5(canvas, unitemp_sensor_getActive(model->sensor_index), ColorWhite, value_positions[3][1], QUARTER);
        _draw_pm_4_0(canvas, unitemp_sensor_getActive(model->sensor_index), ColorWhite, value_positions[3][2], QUARTER);
        _draw_pm_10_0(canvas, unitemp_sensor_getActive(model->sensor_index), ColorWhite, value_positions[3][3], QUARTER);
        break;
    }
}

//TODO: Оптимизировать вывод информации
static void _draw_carousel_info(Canvas* canvas, GeneralViewModel* model) {
    canvas_set_font(canvas, FontPrimary);
    canvas_draw_str(canvas, 10, 23, "Type:");

    if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &ONE_WIRE) {
        OneWireSensor* s = unitemp_sensor_getActive(model->sensor_index)->instance;
        canvas_set_font(canvas, FontPrimary);
        ...
        canvas_draw_str(
            ...
            41,
            23,
            unitemp_onewire_sensor_getModel(unitemp_sensor_getActive(model->sensor_index)));
    ...
    if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &SINGLE_WIRE) {
        ...
        canvas_set_font(canvas, FontSecondary);
        canvas_draw_str(
            canvas, 41, 23, unitemp_sensor_getActive(model->sensor_index)->type->typename);
        canvas_draw_str(
            canvas,
            41,
            35,
    ...
    if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &SPI) {
        ...
        canvas_draw_str(
            ...
            47,
            46,
            ((SPISensor*)unitemp_sensor_getActive(model->sensor_index)->instance)
                ->CS_pin->name);
    ...
    if(unitemp_sensor_getActive(model->sensor_index)->type->interface == &I2C) {
        ...
        snprintf(
            ...
            BUFF_SIZE,
            "0x%02X",
            ((I2CSensor*)unitemp_sensor_getActive(model->sensor_index)->instance)
                    ->currentI2CAdr >>
                1);
...
static void _draw_view_sensorsCarousel(Canvas* canvas, GeneralViewModel* model) {
    //Рисование рамки
    canvas_draw_rframe(canvas, 0, 0, 128, 63, 7);
    canvas_draw_rframe(canvas, 0, 0, 128, 64, 7);
    ...
    canvas_draw_str_aligned(
        ...
        AlignCenter,
        AlignCenter,
        unitemp_sensor_getActive(model->sensor_index)->name);
    //Подчёркивание
    uint8_t line_len =
        canvas_string_width(canvas, unitemp_sensor_getActive(model->sensor_index)->name) + 2;
    canvas_draw_line(canvas, 64 - line_len / 2, 12, 64 + line_len / 2, 12);

    //Стрелка вправо
    if(unitemp_sensors_getTypesCount() > 0 &&
       model->sensor_index < unitemp_sensors_getActiveCount() - 1) {
        canvas_draw_icon(canvas, 122, 29, &I_ButtonRight_4x7);
    }
    //Стрелка влево
    if(model->sensor_index > 0) {
        canvas_draw_icon(canvas, 2, 29, &I_ButtonLeft_4x7);
    }

    switch(model->info_selector) {
    case CAROUSEL_VALUES:
        _draw_carousel_values(canvas, model);
        break;
    case CAROUSEL_INFO:
        _draw_carousel_info(canvas, model);
        break;
    }
}
...
static void _draw_callback(Canvas* canvas, void* _model) {
    ...
    if(model->sensor_index + 1 > sensors_count) model->sensor_index = 0;

    if(sensors_count == 0) {
        model->current_view = G_NO_SENSORS_VIEW;
        _draw_view_noSensors(canvas);
    } else {
        if(sensors_count == 1) model->current_view = G_CAROUSEL_VIEW;
        if(model->current_view == G_NO_SENSORS_VIEW) model->current_view = G_CAROUSEL_VIEW;
        if(model->current_view == G_LIST_VIEW) _draw_view_sensorsList(canvas, model);
        if(model->current_view == G_CAROUSEL_VIEW) _draw_view_sensorsCarousel(canvas, model);
    }
}
...
static bool _input_callback(InputEvent* event, void* context) {
    ...
    if(event->key == InputKeyOk && event->type == InputTypeShort) {
        with_view_model(
            ...
            GeneralViewModel* model,
            {
                ...
                if(model->current_view == G_NO_SENSORS_VIEW) {
                    app->sensors_ready = false;
                    unitemp_SensorsList_switch();
                    consumed = false; // Don't need redraw, we're switching views
                } else if(model->current_view == G_LIST_VIEW) {
                    //Переход в главное меню при выключенном селекторе
                    app->sensors_ready = false;
                    unitemp_MainMenu_switch();
                    consumed = false; // Don't need redraw, we're switching views
                } else if(model->current_view == G_CAROUSEL_VIEW) {
                    app->sensors_ready = false;
                    unitemp_SensorActions_switch(unitemp_sensor_getActive(model->sensor_index));
                    consumed = false; // Don't need redraw, we're switching views
                }
            },
            consumed);
    ...
    if(event->key == InputKeyDown && event->type == InputTypeShort) {
        with_view_model(
            ...
            GeneralViewModel* model,
            {
                ...
                if(model->current_view == G_CAROUSEL_VIEW && model->info_selector == CAROUSEL_VALUES) {
                    model->info_selector = CAROUSEL_INFO;
                    consumed = true;
                }
                //Переход в карусель из списка
                else if(model->current_view == G_LIST_VIEW) {
                    model->current_view = G_CAROUSEL_VIEW;
                    consumed = true;
                }
            },
            consumed);
    ...
    if(event->key == InputKeyUp && event->type == InputTypeShort) {
        with_view_model(
            ...
            GeneralViewModel* model,
            {
                ...
                if(model->current_view == G_CAROUSEL_VIEW && model->info_selector == CAROUSEL_INFO) {
                    model->info_selector = CAROUSEL_VALUES;
                    consumed = true;
                }
                //Переход в список из карусели
                else if(model->current_view == G_CAROUSEL_VIEW && model->info_selector == CAROUSEL_VALUES &&
                   unitemp_sensors_getActiveCount() > 1) {
                    model->current_view = G_LIST_VIEW;
                    consumed = true;
                }
            },
            consumed);
    ...
    if(event->key == InputKeyRight && event->type == InputTypeShort) {
        with_view_model(
            ...
            GeneralViewModel* model,
            {
                ...
                if(model->current_view == G_CAROUSEL_VIEW) {
                    if(++model->sensor_index >= unitemp_sensors_getActiveCount()) {
                        model->sensor_index = 0;
                        if(model->info_selector == CAROUSEL_VALUES) model->current_view = G_LIST_VIEW;
                    }
                    FURI_LOG_I(APP_NAME, "Sensor view index: %d", model->sensor_index);
                    consumed = true;
                }
                //Пролистывание списка вперёд
                else if(model->current_view == G_LIST_VIEW) {
                    model->sensor_index += 4;
                    if(model->sensor_index >= unitemp_sensors_getActiveCount()) {
                        model->sensor_index = 0;
                        model->current_view = G_CAROUSEL_VIEW;
                    }
    ...
    if(event->key == InputKeyBack && event->type == InputTypeShort) {
        ...
        with_view_model(
            ...
            GeneralViewModel* model,
            {
                ...
                if(model->current_view == G_NO_SENSORS_VIEW ||
                   ((model->current_view == G_CAROUSEL_VIEW) && (model->info_selector == CAROUSEL_VALUES))) {
                    app->processing = false;
                    should_exit = true;
                    consumed = false; // Don't consume, let ViewDispatcher handle exit
                }
        ...
        // Return false when exiting to let ViewDispatcher stop properly
        if(should_exit) {
            return false;
        }
        return consumed;
    ...
    if(event->key == InputKeyOk && event->type == InputTypeLong) {
        ...
        // Request redraw to show new temperature unit
        with_view_model(
            view,
            GeneralViewModel* model,
            {
...
static void _tick_callback(void* context) {
    ...
    // Request GUI redraw - the proper way
    // Accessing the model with update flag = true triggers a redraw
    with_view_model(
        view,
        GeneralViewModel* model,
        {
            // No model state changes needed, just triggering redraw
            UNUSED(model);
        },
        true); // true = request GUI system to call draw callback
}

void unitemp_General_alloc(void) {
    view = view_alloc();
    
    // Allocate view model with locking for thread safety
    view_allocate_model(view, ViewModelTypeLocking, sizeof(GeneralViewModel));
    ...
    with_view_model(
        ...
        false); // false = no need to redraw during initialization
...
// Helper functions to allow other views to modify General view state
void unitemp_General_set_carousel_info(carousel_info info) {
    with_view_model(
        view,
        GeneralViewModel* model,
        {
            model->info_selector = info;
        },
        true); // Request redraw
}
